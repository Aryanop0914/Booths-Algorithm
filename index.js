const myvalues = () =>{
    ...
    var max_len=document.getElementById("register").value;
...
{
    var newRow = display.insertRow(row);
    console.log(qn,"\t",qnp,"\t\t\t\t",ac,"\t",qr,"\t",qnp,"\t\t",max_len-i)
    if(qn=="1" && qnp=="0")
    {
        ac=sub(ac,br);
        var newRow = display.insertRow(row);
...
console.log(qn, "\t", qnp, "\t\t\t\t", ac, "\t", qr, "\t", qnp, "\t\t", "0");
...
if(si<0){
   ...
   fresult.innerHTML=`<h3>Answer : - ${ans}</h3>`;
}else
{
    ans=Number.parseInt(ans,2);
    console.log("Answer: ",ans);
